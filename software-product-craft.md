...
They combine the strategic taste of an architect with the hands-on rigor of a principal engineer, treating **design, architecture, build, and maintenance as one continuous activity.** These are never separate phases.
...
- **What it means:** The shape of your data — entities, relationships, lifecycles — constrains every downstream choice: code structure, performance, what features become possible, and what becomes prohibitively expensive.
  - A wrong data model compounds; a right one quietly enables features for years.
  - The real cost is rarely the schema redesign on paper. It is **migrating live data and every dependent that already relies on the old shape** — which is why a model stays hard to change long after its schema file is trivial to edit (see §4, *Migration is a first-class operation*).
- **In practice:**
...
- **Cross-domain instances:**
  ...
  - **Agentic AI (which layer is destiny):** The principle binds the layer with the long half-life, not the one measured in turns. Separate the two before applying it.
    - **Durable substrate** (long-term memory schema, conversation/thread store, persisted tool-result contracts e.g. MCP): *this* is destiny. A retrieval layer keyed only on string match cannot later support semantic similarity without redesigning the memory layer.
    - **Ephemeral working memory** (per-turn orchestrator scratch state, reasoning context for a single request): deliberately disposable. Its value is that it can be rebuilt every turn — applying "destiny" here produces over-engineering, not safety.
...
- **Cross-domain instances:**
  - **Server SaaS:** The "distributed monolith" anti-pattern (Sam Newman, *Building Microservices*, O'Reilly) appears when services are split by data type — `UserService`, `OrderService`, `ProductService` — and every product change requires coordinated deployments across all three.

    Shopify took the inverse path: a single Rails deployable with strict internal module boundaries around business capabilities (Cart, Checkout, Inventory) — decoupling without distribution (Kirsten Westeinde, *Deconstructing the Monolith*, RailsConf 2019).
...
- **Cross-domain instances:**
  ...
  - **Multi-platform SaaS:** The mobile case is harder — app-store review and slow user adoption mean a breaking client/server contract change can take 6+ months to retire. Versioning is not optional.
  - **Workflow / orchestration:** Both major platforms provide explicit versioning to protect in-flight executions:
    - Temporal's Patching API (`workflow.GetVersion` in Go/Java, `workflow.patched` in Python/TypeScript/.NET) records version markers in Event History so replays stay deterministic as code evolves (docs.temporal.io).
...
- **What it means:** Rich Hickey's distinction (*Simple Made Easy*, Strange Loop 2011):
  ...
  Many systems feel "easy" to start with but are not simple. The interleaved concerns surface later as compounding cost.
- **In practice:**
...
- **Cross-domain instances:**
  - **Server SaaS:** ORMs feel easier than raw SQL — `User.find(id).orders.recent` is one line. But beneath it live:
    - Caching, identity maps, lazy loading, eager-loading hints, query interpretation, and transaction management.
    - Teams routinely lose weeks to N+1 queries, stale caches, and surprising transaction boundaries that surface only at scale (Ted Neward, *The Vietnam of Computer Science*, 2006).
...
- **What it means:** Every novel technology in your stack carries hidden, ongoing cost:
  - Hiring, on-call expertise, debugging unknown failure modes, upgrade paths, integration with the rest of the stack.
  ...
  For products whose value proposition *is* novel tech (e.g., AI agents), the rule reframes: **boring substrate, novel surface.** Storage, queues, and observability stay boring; the differentiating layer is where novelty earns its keep.
- **In practice:**
...
- **What it means:** Encode invariants in the type system or schema so that invalid combinations of state cannot be constructed at all. The compiler (or schema validator) enforces correctness, replacing scattered runtime guards.

  Closely related: Alexis King's *Parse, don't validate* (lexi-lambda.github.io, Nov 5, 2019) — convert untrusted input into a typed value once at the boundary, then trust the type from then on.
- **In practice:**
...
- **Resolution (the artifact-half-life axis, §3):**
  ...
  - Stay additively loose on short-half-life surfaces — weekly-changing prompts, skills, internal DTOs — where new variants land often and the set of legal states is still moving.
...
- **What it means:** Gary Bernhardt's pattern from *Boundaries* (SCNA 2012):
...
- **Cross-domain instances:**
  ...
  - **Workflow / orchestration:** Temporal persists an append-only Event History per workflow. On worker crash, the code replays against that history to reconstruct in-memory state deterministically. Workflow code is the pure core; Activities are the imperative shell — the pattern maps directly.
...
- **What it means:** Martin Fowler's pattern (*StranglerFigApplication*, martinfowler.com, June 2004), named after the strangler fig tree that grows around its host. Route traffic incrementally to the new implementation while the old keeps serving, until the new carries all traffic and the old can be removed.

  Big-bang rewrites fail at very high rates: they require freezing a moving target and demand perfect knowledge of the old system's behavior.
- **In practice:**
...
- **What it means:** Failure-mode design (§3.8) asks *"what breaks?"* Adversarial design asks *"what can be made to break by someone who wants it to?"* The two produce different mitigations:
  ...
  Distinguished engineers treat security as a design input, not a security-team handoff.
- **In practice:**
...
- **Cross-domain instances:**
  - **Server SaaS (cautionary):** The Equifax 2017 breach exposed personal information of approximately 145.5 million U.S. consumers (GAO-18-559, *Data Protection: Actions Taken by Equifax and Federal Agencies in Response to the 2017 Breach*, September 2018).
    - Apache Struts CVE-2017-5638 was patched on March 7, 2017.
    - Attackers first accessed Equifax systems on May 13, 2017; the breach wasn't detected until July 29.
    - The patch had been available for two months before exploitation — the system that should have applied it wasn't.
  - **Supply chain (cautionary):** SolarWinds SUNBURST (disclosed December 13, 2020; CISA Alert AA20-352A) compromised the build pipeline for Orion software updates released March–June 2020. Up to 18,000 customers downloaded the trojanized update (SolarWinds SEC filing). The threat model assumed attackers target the application; this attack went a layer below.
  - **Agentic AI:** Prompt injection is the new SQL injection.
    - The term was coined by Simon Willison (*Prompt injection attacks against GPT-3*, simonwillison.net, September 12, 2022), building on an exploit first surfaced by Riley Goodside.
    - In February 2023, Stanford student Kevin Liu extracted Bing Chat's hidden system prompt using a direct prompt injection, exposing its internal codename "Sydney."

    The defense is identical to SQL injection: never trust input reaching an interpreter; isolate untrusted content from privileged operations.
